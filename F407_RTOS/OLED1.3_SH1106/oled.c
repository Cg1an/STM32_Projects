...
//	I2C_Stop();
	  ...
	  if(mode){MEM=0x40;}
		...
		HAL_I2C_Mem_Write(&hi2c1,0x78,MEM,I2C_MEMADD_SIZE_8BIT,&dat,1,0XFF);
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
		for(n=0;n<128;n++)
		{
			OLED_MEM[n]=OLED_GRAM[n][i];
		}
		OLED_WR_Byte(0xb0+i,OLED_CMD); //设置行起始地址
		OLED_WR_Byte(0x02,OLED_CMD);   //设置低列起始地址
		OLED_WR_Byte(0x10,OLED_CMD);   //设置高列起始地址
	  uint8_t MEM=0x40;
		HAL_I2C_Mem_Write(&hi2c1,0x78,MEM,I2C_MEMADD_SIZE_8BIT,OLED_MEM,128,0XFF);
...
//		I2C_Stop();
		 //   uint8_t MEM=0x40;
//		HAL_I2C_Mem_Write(&hi2c1,0x78,MEM,I2C_MEMADD_SIZE_8BIT,OLED_MEM,128,1000);
