...
{
  /* USER CODE BEGIN 1 */
  0x06, 0x00, 0xFF,  // Usage Page (Vendor Defined 0xFF00)//供应商定义
...
static int8_t CUSTOM_HID_OutEvent_HS(uint8_t event_idx, uint8_t state)
{
  ...
  UNUSED(state);
	...
	size = USBD_GetRxCount( &hUsbDeviceHS,CUSTOM_HID_EPOUT_ADDR );
	USBD_CUSTOM_HID_HandleTypeDef *hhid = (USBD_CUSTOM_HID_HandleTypeDef *)(hUsbDeviceHS.pClassData);
	
	memcpy(USB_RX_buffer,hhid->Report_buf,64);
//  for(int i=0; i<size; i++)
//  {
//	   USB_RX_buffer[i]=hhid->Report_buf[i]; // 读取接收到的数据
//	   usbbuff[i+1]=hhid->Report_buf[i]; // 读取接收到的数据 
//  }
	//HAL_GPIO_TogglePin(LED4_GPIO_Port,LED4_Pin);
  Analyze_USB_DATA();
	usbbuff[0]=0;
 // USBD_CUSTOM_HID_SendReport(&hUsbDeviceHS, usbbuff, 64);
