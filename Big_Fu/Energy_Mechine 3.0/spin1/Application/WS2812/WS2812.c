...
//void WS_WriteAll_RGB_REC_R(uint8_t n_R, uint8_t n_G, uint8_t n_B )
//{
//  uint8_t red=n_R;
//  uint8_t green=n_G;
//  uint8_t blue=n_B;
...
/*
   ...
   stage: 当前激活组数 (1~5)
*/
void WS_Write_Progress_REC(uint8_t stage, uint8_t n_R, uint8_t n_G, uint8_t n_B)
{
    uint16_t n = 0;
    // 根据头文件 PIXEL_NUM_REC_ 宏定义(不含R标的箭头长度, 设为256)，将其分成5等份
    uint16_t light_num = (PIXEL_NUM_REC_ / 5) * stage; 
    
    if (stage >= 5) light_num = PIXEL_NUM_REC_; // 第5组通关，灯带满载

    for(n = 0; n < PIXEL_NUM_REC; n++) // 遍历整条中心灯带
    {
        if (n < light_num)
        {
            // 在进度比例以内的灯珠：亮起指定颜色
        ...
        {
            // 超出进度比例的灯珠：熄灭
            WS281x_SetPixelColor_RECT(n, 0, 0, 0);       
        }
    }
    
    WS_Load_rectangle(); // 发送 DMA 刷新底层波形
}
