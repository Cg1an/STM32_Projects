    ...
    while (1)
    {
        ...
        if (!Board_Order_info[BOARD_NUMBER].RainBow_Flag) //不为流水效果
        {
            ...
            if (Board_Order_info[BOARD_NUMBER].Board_Work == 0)
            {
                ...
                {
                    // 如果已有激活进度，根据当前阵营颜色点亮对应的比例
                    if (Big_Fu_info[BOARD_NUMBER].Color == BLUE)
                    {
                        WS_Write_Progress_REC(current_stage, 0, 0, BLUE_NUM);
                    }
                    else if (Big_Fu_info[BOARD_NUMBER].Color == RED)
                    {
                        WS_Write_Progress_REC(current_stage, RED_NUM, 0, 0);
                    }
                }
                else
                {
                    // 如果 count_t 为 0（还没开始打，或者重置了），中间灯带全灭
                    WS_WriteAll_RGB_REC(0, 0, 0);
                }
