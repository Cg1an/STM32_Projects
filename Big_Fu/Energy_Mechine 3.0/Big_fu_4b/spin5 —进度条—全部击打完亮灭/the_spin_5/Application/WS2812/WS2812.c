...
void WS_WriteAll_RGB_FRAME(uint8_t n_R, uint8_t n_G, uint8_t n_B )
{
	...
	//PIXEL_NUM_FRAME
	//flag_color
	for(n =0 ; n <PIXEL_NUM_FRAME; n++) //PIXEL_NUM_FRAME
	{
...
/*
 * 独立封装的 R 标显示函数
 * 作用：在数组的第 0~63 号灯珠（即物理上第 1~64 个灯珠）上绘制 R 标
 * 参数：n_R, n_G, n_B 分别为红、绿、蓝的亮度值
 */
void WS_Write_R_Logo(uint8_t n_R, uint8_t n_G, uint8_t n_B)
{
    uint16_t idx;
    uint8_t i;
    
    // 遍历前 64 个灯珠 (数组下标 0 到 63)
    for (idx = 0; idx < 64; idx++)
    {
        // 判断当前下标是否在 R 标的笔画范围内 (原坐标全部减去 256)
        if ((idx >= 0  && idx < 7)  || 
            (idx >= 8  && idx < 10) || 
            (idx >= 15 && idx < 18) || 
            (idx >= 23 && idx < 31) || 
            ...
            (idx >= 61 && idx < 63))
        {
            // 【笔画区域】：填入指定的 RGB 颜色
            for (i = 0; i < 24; ++i) {
                send_Buf_rectangle[24 * idx + i] = (((WS281x_Color(n_R, n_G, n_B) << i) & 0X800000) ? WS1 : WS0);
            }
        }
        else
        {
            // 【空白间隙】：强制填入黑色(熄灭)，防止出现杂色重影
            for (i = 0; i < 24; ++i) {
                send_Buf_rectangle[24 * idx + i] = (((WS281x_Color(0, 0, 0) << i) & 0X800000) ? WS1 : WS0);
            }
        }
    }
    
    // 发送 DMA 底层波形刷新 (如果您想在调用该函数后立刻亮起，请取消注释这行)
    WS_Load_rectangle(); 
}
