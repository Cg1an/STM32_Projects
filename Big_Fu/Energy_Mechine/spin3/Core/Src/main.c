...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
  {	
    if(HAL_GPIO_ReadPin(GPIOC,GPIO_PIN_8)==0)chen1++;

		if(chen1<200)
WS_WriteAll_RGB_FRAME(RED_NUM,0,0);//À¶µÆ

//    if(chen1>200)
//WS_WriteAll_RGB_FRAME(0,0,0);//µÆÃð

		if(chen1>300)
WS_WriteAll_RGB_FRAME(0,0,BLUE_NUM);//ºìµÆ
		HAL_Delay(5);
}
}
