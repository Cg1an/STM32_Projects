...
//mar:存储器地址
//ndtr:数据传输量  
void MYDMA_Config(DMA_Stream_TypeDef *DMA_Streamx,u8 chx, u8 dir, u32 par)
{
	DMA_TypeDef *DMAx;
	u8 streamx;
	...
	streamx=(((u32)DMA_Streamx-(u32)DMAx)-0X10)/0X18;		//计算出stream通道号
	...
	DMA_Streamx->CR|=1<<4;		//使能DMA传输完毕中断
	DMA_Streamx->CR|=dir<<6;	//数据传输方向（0：存储器到外设模式；1：存储器到外设模式；2：存储器到存储器模式）
...
void MYDMA_Enable(DMA_Stream_TypeDef *DMA_Streamx,u32 mar,u16 ndtr)
{
	...
	DMA_Streamx->NDTR=ndtr;		//一次传输所发送的数据个数
